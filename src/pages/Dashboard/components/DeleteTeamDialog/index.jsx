...
const DeleteTeamDialog = ({ dialogOpen, setDialogOpen, team, setMyTeams }) => {
	...
	const handleSubmit = (e) => {
		e.preventDefault();
		setIsLoading(true);
		TeamsService.deleteTeam(team?.id)
			.then((res) => {
				TeamsService.getOwnedTeams(userId, 1).then((result) => {
					setMyTeams(result.data);
				});
				TeamsService.getAllOwnedTeams(userId)
					.then((result) => {
						setUser({
							ownTeams: result.data,
						});
					});
