...
export const getAllOwnedTeams = async (userId) => apiClient.get(`/users/${userId}/teams/all`);
...
export const getSharedTeams = async (page = 1) => apiClient.get(`/teams?page=${page}`);

export const getSingleTeam = async (teamId) => apiClient.get(`/teams/${teamId}`);

export const createTeam = async (userId, data) =>
  apiClient.post(`/users/${userId}/teams`, { ...data });

export const deleteTeam = async (teamId) => apiClient.remove(`/teams/${teamId}`);

export const updateTeam = async (userId, teamId, data) =>
  apiClient.put(`/users/${userId}/teams/${teamId}`, { ...data });

export const inviteUsers = async (teamId, users) =>
  apiClient.post(`/teams/${teamId}/invite`, { users });
